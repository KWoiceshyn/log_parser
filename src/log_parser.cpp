...
LogParser::LogParser()
    : log_file_path_(""), code_OK_("200"), target_request_("GET") {}
...
void LogParser::processLine(const string &line) {
  ...
  if (start_time_) {
    ...
    chrono::system_clock::time_point line_time =
...
void LogParser::setStartTime(const string &start_time) {
  ...
  start_time_ = make_unique<chrono::system_clock::time_point>(
      chrono::system_clock::from_time_t(mktime(&tm)));
}
...
void LogParser::setEndTime(const string &end_time) {
  ...
  ss >> get_time(&tm, date_input_format_);
  end_time_ = make_unique<chrono::system_clock::time_point>(
      chrono::system_clock::from_time_t(mktime(&tm)));
}

vector<pair<string, int>>
LogParser::sortByCount(const unordered_map<string, int> &data) {
